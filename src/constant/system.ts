const URL = 'https://norma.nomoreparties.space/api';

export const DATA_URL = `${URL}/ingredients`;

export const ORDER_URL = `${URL}/orders`;

export const PASSWORD_RESET_URL = `${URL}/password-reset`;

export const PASSWORD_RESET_SAVE_URL = `${URL}/password-reset/reset`;

export const AUTH_REGISTER_URL = `${URL}/auth/register`;

export const AUTH_LOGIN_URL = `${URL}/auth/login`;

export const AUTH_LOGOUT_URL = `${URL}/auth/logout`;

export const AUTH_TOKEN_URL = `${URL}/auth/token`;

export const AUTH_USER_URL = `${URL}/auth/user`;
